...
description: "Gobarau Backend enforces role-based access with ten predefined roles and four custom DRF permission classes covering admin, staff, teacher, and parent access levels."
---
...
Extends `IsAdminLevel` to also permit `bursar` and `teacher` roles. Use this on ViewSets where operational staff — not just leadership — need read or write access, such as student lists, class schedules, and finance summaries.
