...
description: "REST endpoints for Gobarau Academy's public-facing content: school news, events calendar, photo gallery albums, magazine issues, annual yearbook, and testimonials."
---

The Content module powers the public-facing side of Gobarau Academy's platform — everything a prospective family, current student, or returning alumnus might browse on the school website. It provides structured endpoints for publishing school news articles, listing upcoming events, organising photo galleries, distributing magazine issues and their articles, commemorating graduating classes through the yearbook, and showcasing testimonials from parents and alumni. All `GET` and `retrieve` actions on every content endpoint are fully public (no authentication required), while `POST`, `PUT`, `PATCH`, and `DELETE` operations require an authenticated staff or admin user. Image and file uploads are handled via Cloudinary through Django's `cloudinary_storage` backend.

<Note>
  All image and file fields (`featured_image`, `cover_image`, `image`, `photo`, `pdf_url`, etc.) are stored on **Cloudinary** via `django-cloudinary-storage`. Send these as `multipart/form-data` requests when uploading files, and expect a Cloudinary CDN URL back in the response.
</Note>
...
<ParamField body="featured_image" type="file">
  Optional hero image for the article. Upload as `multipart/form-data`; stored on Cloudinary.
</ParamField>
...
<ResponseField name="featured_image" type="string | null">
  Cloudinary CDN URL of the uploaded image, or `null`.
</ResponseField>
...
<ParamField body="cover_image" type="file">
  Optional cover image. Upload as `multipart/form-data`; stored on Cloudinary.
</ParamField>
...
<ResponseField name="cover_image" type="string | null">
  Cloudinary CDN URL of the cover image, or `null`.
</ResponseField>
...
<ParamField body="cover_image" type="file">
  Thumbnail/cover image for the album. Upload as `multipart/form-data`; stored on Cloudinary.
</ParamField>
...
<ParamField body="image" type="file" required>
  The photo to upload. Send as `multipart/form-data`; stored on Cloudinary under `content/gallery/images/`.
</ParamField>
...
<ResponseField name="image" type="string">
  Cloudinary CDN URL of the uploaded photo.
</ResponseField>
...
Magazine issues represent individual editions of the school's publication. Each issue has a unique number and volume identifier, a publication date, an optional cover image, and an optional PDF file hosted on Cloudinary.
...
<ParamField body="cover_image" type="file">
  Magazine cover image. Upload as `multipart/form-data`; stored on Cloudinary.
</ParamField>

<ParamField body="pdf_url" type="file">
  Full PDF of the magazine issue. Upload as `multipart/form-data`; stored on Cloudinary under `content/magazines/pdfs/`.
</ParamField>
...
<ResponseField name="id" type="uuid">
  Unique identifier for the magazine issue.
</ResponseField>
...
<ResponseField name="pdf_url" type="string | null">
  Cloudinary CDN URL of the PDF file, or `null`.
</ResponseField>
...
<ParamField body="cover_image" type="file">
  Cover image for the yearbook. Upload as `multipart/form-data`; stored on Cloudinary.
</ParamField>
...
<ResponseField name="id" type="uuid">
  Unique identifier for the yearbook set.
</ResponseField>
...
<ParamField body="photo" type="file">
  Student portrait for the yearbook page. Upload as `multipart/form-data`; stored on Cloudinary under `content/yearbooks/students/`.
</ParamField>
...
<ResponseField name="id" type="uuid">
  Unique identifier for the yearbook entry.
</ResponseField>
...
<ResponseField name="photo" type="string | null">
  Cloudinary CDN URL of the student portrait, or `null`.
</ResponseField>
...
<ParamField body="photo" type="file">
  Optional portrait of the testimonial author. Upload as `multipart/form-data`; stored on Cloudinary under `content/testimonials/`.
</ParamField>
...
<ResponseField name="photo" type="string | null">
  Cloudinary CDN URL of the author's portrait, or `null`.
</ResponseField>
